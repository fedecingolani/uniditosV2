...
import 'package:uniditos/pages_phone/reportes/generar_pdf/recibo_pago.dart';
...
class DialogNewComprobante extends StatefulWidget {
  final int? alumnoId;

  const DialogNewComprobante({Key? key, this.alumnoId}) : super(key: key);

  @override
  State<DialogNewComprobante> createState() => _DialogNewComprobanteState();
}

class _DialogNewComprobanteState extends State<DialogNewComprobante> {
  ...
  _saveComprobante() {
    if (_formKey.currentState!.validate()) {
      ...
      var numero = DateTime.now().millisecond;
      ComprobantesService.createComprobante(
            Comprobantes(
              id: 0,
              alumnoId: _selectedAlumno!.alumnoId,
              fecha: _selectedFecha,
              importe: _importe.toInt(),
              letraComprobante: 'P',
          ...
          .then((value) {
            if (value.statusCode == 200 || value.statusCode == 201) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Comprobante agregado')),
              );
              String concepto = 'Pago de cuota/Comedor';
              String recibo = 'P-$numero';
              generateReciboPago(
                _importe.toInt(),
                recibo,
                concepto,
                _selectedAlumno!.alumnoId,
              );
            } else {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Error al agregar comprobante')),
              );
            }
          ...
          .catchError((e) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('Error al agregar comprobante: ${e.toString()}'),
              ),
            );
          });

      // ComedorService.createMovimientoComedor(
      //   ComedorCreate(alumnoId: _selectedAlumno!.alumnoId, fecha: _selectedFecha, importe: _importe, pago: false),
      // ).then((value) {
      //   if (value.statusCode == 200 || value.statusCode == 201) {
      //     ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Alumno agregado')));
      //   } else {
      //     ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Error al agregar comedor')));
      //   }
      //   Navigator.of(context).pop();
      // }).catchError((e) {
      //   ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error al agregar comedor: ${e.toString()}')));
      // });
    }
  }
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: const Text('Agregar Comprobante de Pago'),
      content: Form(
        key: _formKey,
        child: Padding(
